data {
  int N; //the number of observations — subjects
  int K; //the number of columns in the model matrix — TCRs
  int y[N]; //the response — CMV status 
  matrix[N,K] X; //the model matrix -- subj vs TCR data
  int W[N]; //the number of trials per observations — vector of 1's (TCR present) for each subject  
}
parameters {
